/**
 * PLAYER-2-SIM — Jogador + Segredo
 * Simula um jogador que recebe mensagem secreta
 *
 * Uso: SESSION_CODE=<codigo> node server/virtual-table-player-2.mjs
 */
...
async function main() {
  ...
  try {
    ...
    player.log(`Usando código: ${sessionCode}`)
    ...
    while (stageCount < maxStages) {
      try {
        ...
        if (stageMsg.stage.id === 'stage-secret-2' || (stageMsg.stage.audience?.kind === 'participants' && stageMsg.stage.audience.participantIds?.includes(player.participantId))) {
          ...
          player.log(`   Audiência confirmada: apenas para este jogador (participantIds: ${stageMsg.stage.audience.participantIds?.join(', ')})`)
        }

        // Verificar audience de todas as cenas
    ...
    if (summary.secretContent) {
      console.log(`  Conteúdo do segredo: ${summary.secretContent}`)
    }
    console.log('═══════════════════════════════════════════\n')
  }
}
