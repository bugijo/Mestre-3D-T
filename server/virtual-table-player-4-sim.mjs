/**
 * PLAYER-4-SIM — Jogador Adversarial (Simulation Version)
...
class TestClient {
  ...
  waitForError(actionId, timeout = 3000) {
    return this.waitFor(m => m.type === 'error' && (m.actionId === actionId || (m.message && m.message.includes(actionId))), timeout)
  }
...
async function main() {
  ...
  try {
    ...
    if (resp2 && resp2.code === 'FORBIDDEN') {
      ...
      player.log(`⚠️ ACEITO (ack): duplicate=${ack2.duplicate}`)
    } else {
